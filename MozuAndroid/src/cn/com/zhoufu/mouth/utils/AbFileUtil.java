...
import cn.com.zhoufu.mouth.constants.AbAppData;
...
public class AbFileUtil {
	 ...
	 * @return Bitmap 新图片
	 */
	public static Bitmap getBitmapFromSDCache(String url, int type,
		...
		try {
			...
			if (type != AbConstant.ORIGINALIMG
	 ...
	 * @return Bitmap 新图片
	 */
	public static Bitmap getBitmapFromSD(String url, int type, int newWidth,
			int newHeight) {
		Bitmap bit = null;
		try {
			// SD卡是否存在
	 ...
	 * @return Bitmap 新图片
	 */
	public static Bitmap getBitmapFormByte(byte[] imgByte, String fileName,
			int type, int newWidth, int newHeight) {
	 ...
	 *            处理类型.
	 */
	public static String getImageFileName(String url, int width, int height,
			int type) {
		return AbMd5.MD5(new StringBuilder(url.length() + 12).append("#W")
